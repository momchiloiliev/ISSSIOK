...
{
    session_start();
    $cookieValue = session_id() . '|' . time() . '|' . $username;
    setcookie('session_cookie', $cookieValue, time() + 60 * 60);
}
...
if (isset($_COOKIE['session_cookie'])) {
    list($sessionId, $lastAccessTime, $storedUsername) = explode('|', $_COOKIE['session_cookie']);
    $currentTime = time();

    if ($currentTime - $lastAccessTime < 2 * 60 * 60) {
        $output .= "<p class='output'>Hello $storedUsername!</p>";
    }

    if ($currentTime - $lastAccessTime > 60 * 60) {
        $output .= "<p class='output'>Hello $storedUsername, you are still logged in!</p>";
    }
}
